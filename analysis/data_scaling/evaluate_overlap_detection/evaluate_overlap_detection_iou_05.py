"""Overlap detection (IoU > 0.5) vs. training size for the data-scaling experiment.

Uses the canonical prediction dataset's retained-component overlap flags and
...
Run from the project root:
    python analysis/data_scaling/evaluate_overlap_detection/evaluate_overlap_detection_iou_05.py
"""
...
OUT_BASE = "evaluate_overlap_detection_iou_05"
...
PLOT_SERIES = [
    ("combined", "Combined detection", "#2a78d6"),
    ("malignant", "Malignant detection", "#eb6834"),
    ("benign", "Benign detection", "#3b6d11"),
]
            ...
            for row in run:
                if row.pathology in ("malignant", "benign"):
                    if not row.mass_present or getattr(row, FLAG_FIELD) is None:
                        raise DatasetValidationError(f"Invalid overlap case: {row.image_id}")
...
def summarize_sizes(runs, positive_class):
    ...
    for size in SIZES:
        results = [evaluate_detection(runs[size, seed], positive_class) for seed in SEEDS]
...
def main():
    ...
    title = f"Case-level overlap-based detection (IoU>{IOU_THRESHOLD:g}) by training size"
    description = (
        "Data-scaling test runs: seeds 42/43/44, final checkpoints, "
        f"training sizes {', '.join(map(str, SIZES))}. Detection = IoU > "
        f"{IOU_THRESHOLD:g}, noise floor "
    ...
    lines = [
        ...
        f"A mass-present case is detected when `{FLAG_FIELD}` is True: the highest "
        "single-component IoU between a retained predicted component and the "
        f"ground-truth mass exceeds {IOU_THRESHOLD:g}. Off-target blobs are ignored.", "",
