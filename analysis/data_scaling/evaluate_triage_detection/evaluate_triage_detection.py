"""Triage detection vs. training size for the data-scaling experiment.

Uses the canonical prediction dataset's triage flags (any retained mass,
no overlap required). Outputs terminal tables, Markdown, JSON, PNG, and
PDF with mean ± sample SD across seeds 42/43/44 at final checkpoints.

Run from the project root:
    python analysis/data_scaling/evaluate_triage_detection/evaluate_triage_detection.py
"""
...
OUT_BASE = "evaluate_triage_detection"
...
PLOT_SERIES = [
    ("combined", "Combined triage", "#2a78d6"),
    ("malignant", "Malignant triage", "#eb6834"),
    ("benign", "Benign triage", "#3b6d11"),
]
...
def select_runs(rows):
    ...
    for size in SIZES:
        for seed in SEEDS:
            ...
            for row in run:
                if not isinstance(row.triage_detection_flag, bool):
                    raise DatasetValidationError(f"Invalid triage flag: {row.image_id}")
...
def evaluate_triage(rows, positive_class):
    ...
    recall = mean(row.triage_detection_flag for row in positives)
    fp_rate = mean(row.triage_detection_flag for row in normal_cases)
...
def summarize_sizes(runs, positive_class):
    ...
    for size in SIZES:
        results = [evaluate_triage(runs[size, seed], positive_class) for seed in SEEDS]
...
def main():
    ...
    title = "Case-level triage-based detection by training size"
    description = (
        "Data-scaling test runs: seeds 42/43/44, final checkpoints, "
        f"training sizes {', '.join(map(str, SIZES))}. Triage = any retained mass "
        "(no overlap required), noise floor "
    ...
    lines = [
        ...
        "anywhere on the image. No overlap or IoU threshold is required.", "",
