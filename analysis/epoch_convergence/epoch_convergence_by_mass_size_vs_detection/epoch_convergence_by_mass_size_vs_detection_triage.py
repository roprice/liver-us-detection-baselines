"""Triage detection vs. epoch, broken down by mass size, for the preliminary
milestones run (3 seeds, 625 images each).
...
Mass-present cases are split into 4 area bins on ground_truth_mass_area_px
(0-3000, 3001-10000, 10001-30000, 30000+). For each bin, and per mass grouping
(combined / malignant / benign), this reports case-level recall and the
false-positive rate over Normal cases, averaged across the three seeds with
standard deviation. Snapshot (best / best mass / final) checkpoints are reported
as extra rows.
...
than the raw masks; triage_detection_flag, pathology, and
ground_truth_mass_area_px come from there.

Three seeds (42/43/44), 625 images each; error bars are the across-seed std.

Run from project root:
    python analysis/epoch_convergence/epoch_convergence_by_mass_size_vs_detection/epoch_convergence_by_mass_size_vs_detection_triage.py
"""
...
OUT_BASE = "epoch_convergence_by_mass_size_vs_detection_triage"
...
def evaluate_triage_bin(rows, positive_class, low, high=None):
    ...
    mass, so they do not belong to any size bin).
    """
    tp = fn = 0
    fp = 0
    n_normal = 0
...
def evaluate_epoch_bin(rows, positive_class, low, high=None):
    """Run triage eval for one checkpoint and bin; return recall and FP rate."""
    ctp, cfp, cfn, n_normal = evaluate_triage_bin(rows, positive_class, low, high)
...
def main():
    ...
    payload = {
        "title": "Case-level triage-based detection by saved milestone epoch and mass size",
        "description": ("Three preliminary milestones test runs: seeds 42/43/44, "
                        "625 images each, triage = any retained mass (no overlap "
                        f"required), noise floor {NOISE_FLOOR}. Mass-present cases "
                        "split by ground-truth mass area into four bins. Values are "
    ...
    md_lines = [
        ...
        "",
        "Mass into 4 bins according to size and triage detection is measured against "
        "each by epoch",
    ]
    for label, low, high in MASS_BINS:
    ...
    for title, group in [("Both pathologies, 4 log-sized area bins", "combined"),
        ...
        for label, low, high in MASS_BINS:
            ...
            md_lines.append("| Checkpoint | Triage Detection | Normal cases FP rate |")
            md_lines.append("|------:|----------:|-------------------:|")
            for i, ep in enumerate(epochs):
                md_lines.append(
    ...
    # --- Plot: single chart, combined triage recall vs epoch, one line per bin ---
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Combined triage recall')
