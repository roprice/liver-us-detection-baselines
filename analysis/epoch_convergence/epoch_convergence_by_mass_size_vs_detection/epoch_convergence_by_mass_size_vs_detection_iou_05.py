"""Overlap detection (IoU > 0.5) vs. epoch, broken down by mass size, for the
preliminary milestones run (3 seeds, 625 images each).

Overlap detection matches predicted components to the ground-truth mass by
spatial overlap: a case is detected when the highest single-component IoU
between a retained predicted component and the ground-truth mass exceeds 0.5.
...
than the raw masks; overlap_detection_iou_05_flag, normal_false_positive,
...
Run from project root:
    python analysis/epoch_convergence/epoch_convergence_by_mass_size_vs_detection/epoch_convergence_by_mass_size_vs_detection_iou_05.py
"""
...
IOU_THRESHOLD = 0.5
FLAG_FIELD = "overlap_detection_iou_05_flag"
OUT_BASE = "epoch_convergence_by_mass_size_vs_detection_iou_05"
...
def evaluate_overlap_bin(rows, positive_class, low, high=None):
...
def evaluate_epoch_bin(rows, positive_class, low, high=None):
    """Run overlap (IoU > 0.5) eval for one checkpoint and bin; return recall and FP rate."""
    ctp, cfp, cfn, n_normal = evaluate_overlap_bin(rows, positive_class, low, high)
...
def main():
    ...
    md_lines = [
        ...
        f"detection = IoU > 0.5, noise floor {NOISE_FLOOR}.",
        "",
        ...
        "centroid detection.",
        "",
        "Mass into 4 bins according to size and overlap detection is measured "
    ...
    for title, group in [("Both pathologies, 4 log-sized area bins", "combined"),
        ...
        for label, low, high in MASS_BINS:
            ...
            md_lines.append("| Checkpoint | Overlap Detection | Normal cases FP rate |")
            md_lines.append("|------:|----------:|-------------------:|")
            for i, ep in enumerate(epochs):
                md_lines.append(
    ...
    # --- Plot: single chart, combined overlap recall vs epoch, one line per bin ---
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Combined overlap recall')
