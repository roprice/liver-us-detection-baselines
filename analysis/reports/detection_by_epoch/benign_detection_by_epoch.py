"""Benign-mass detection vs. epoch, seven criteria on one chart.

Plots benign case-level recall across two detection families plus triage,
...
Benign detection is the case-level recall (fraction of benign cases detected).

Single-seed run (seed 42, 625 images, joint-selected checkpoints); no error bars.

Run from project root:
    python analysis/reports/detection_by_epoch/benign_detection_by_epoch.py
"""
...
def load_recall(rel_path):
    """Return (epochs, benign case_recall) from a criterion JSON."""
    path = ANALYSIS_ROOT / rel_path
    with open(path) as f:
        data = json.load(f)
    return data["epochs"], data["benign"]["Detection"]
...
def main():
    ...
    md_lines = [
        ...
        "",
        "Values are benign case-level recall (fraction of benign cases "
        "detected) under seven criteria: triage, three overlap IoU tiers, and "
        "three centroid tolerance tiers.",
        "",
    ...
    md_lines.append("")
    md_path = OUT_DIR / "benign_detection_by_epoch.md"
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Benign detection (case recall)')
    ...
    for ext in ('pdf', 'png'):
        out = OUT_DIR / f'benign_detection_by_epoch.{ext}'
