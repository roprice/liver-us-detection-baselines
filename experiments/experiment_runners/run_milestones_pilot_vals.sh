...
#
# Prerequisites:
#   - A local run_milestones_pilot_vals.zip archive containing
...
# =====================================================================
ARCHIVE_PATH="${1:-${HOME}/run_milestones_pilot_vals.zip}"
if [ ! -f "$ARCHIVE_PATH" ]; then
    echo "ERROR: Archive not found: ${ARCHIVE_PATH}"
    echo "Upload run_milestones_pilot_vals.zip or pass its path as the first argument."
    exit 1
fi
